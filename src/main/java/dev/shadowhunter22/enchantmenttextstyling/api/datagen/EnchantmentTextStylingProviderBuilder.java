...
public class EnchantmentTextStylingProviderBuilder {
	 ...
	 * }}</pre>
	 *
	 * <p>If, for example, a value of {@code 2} is also supplied to {@link EnchantmentStyling.Builder#max(int)}, then it will
	 * evaluate as follows:
	 *
	 ...
	 * }}</pre>
	 *
	 * <p>If, for example, a value of {@code 1} is also supplied to {@link EnchantmentStyling.Builder#min(int)}, then it will
	 * evaluate as follows:
	 *
	 * <pre>{@code if (level >= 1 && level <= 2) {
	 *     // apply text styling
